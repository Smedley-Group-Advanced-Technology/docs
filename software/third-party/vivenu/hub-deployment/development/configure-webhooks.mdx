...
- The HMAC key for this hub generated and recorded in the database configuration (covered in a separate SOP).
...
<Steps>
  ...
  <Step title="Set the HMAC key">
    Take the HMAC key recorded in the database configuration for this hub, **base64 encode** it, and paste the encoded value into the **HMAC Key** field.

    <Warning>
      Do not use the **Generate HMAC Key** button in Vivenu. The key must match the value stored in the database configuration so the receiving API can validate incoming webhooks.
    </Warning>
  </Step>
